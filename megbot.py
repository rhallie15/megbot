...
class MegBot:
	'''
	   MegBot is an interface to Facebook Messages. 
	   You can use it by logging into your account, and giving it a group message
	   you are a member of. You can navigate to different pages of the message,
	   read the page, and write to the group chat. Please make sure the other
	   members are aware MegBot is joining y'all in your conversation.
	'''
        ...
        def read_messages(self):
			...
			for line in lines:
				l = filter(lambda x: x in string.printable,
						   line.text.strip());
				if (l and (l != ".") and (not ("Sent from" in l))
					and (not ("Seen by" in l))):
					messages.insert(0, l);
				else:
					print "skipped " + l;
